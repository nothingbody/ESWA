...
def test_advanced_alns_keeps_solution_feasible(tmp_path: Path) -> None:
    path = tmp_path / "sample.txt"
    path.write_text(SAMPLE_SOLOMON, encoding="utf-8")
    task = parse_solomon(path)
    solution = greedy_insertion(task)
    improved = advanced_alns(task, solution, AdvancedAlnsConfig(iterations=20, seed=3, time_limit=0))
